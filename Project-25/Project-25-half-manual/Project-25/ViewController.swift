...
class ViewController: UICollectionViewController, UIImagePickerControllerDelegate, UINavigationControllerDelegate {
    ...
    /*func joinSession(_ ac: UIAlertAction) {
        ...
        guard let browse = browse else { return }
        browse.startBrowsingForPeers()
        let broserUI = MCBrowserViewController(browser: browse, session: peerSession)
        broserUI.delegate = self
        present(broserUI, animated: true)
    }*/
    
    func joinSession(_ ac: UIAlertAction) {
        guard let peerSession = peerSession else { return }
        let broserUI = MCBrowserViewController(serviceType: "abc-phong", session: peerSession)
        broserUI.delegate = self
        present(broserUI, animated: true)
    }
}
